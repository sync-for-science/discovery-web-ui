...
export default class Search extends React.Component {
   ...
   indexData = () => {
     const tree = {
       match: '', complete: [], next: [], refs: [],
     };

     // Index the resources for this participant
     for (const res of this.props.data) {
       if (res.category !== 'Patient' && !Unimplemented.unimplementedCats.includes(res.category)) {
         this.indexResource(tree, res.data, {
           resource: res, provider: res.provider, category: res.category, date: res.itemDate, veryInteresting: false,
         });
       }
     }
     const terms = `${tree.next.reduce((accum, elt) => accum + elt.complete.length, 0)} terms`;
     const totalRefs = tree.next.reduce((accum, elt) => accum + elt.refs.length, 0);
     this.setState({
       searchTree: tree, searchTerms: terms, searchStatus: terms, totalSearchRefs: totalRefs,
     });
   }
     ...
     switch (typeof res) {
       ...
       case 'string':
         ...
         if (resStr.length > 0) {
           ...
           for (const word of resStr.split(' ')) {
             const cleanWord = word.replace(',', '').replace('[', '').replace(']', '');
   ...
   addToTree(tree, level, value, dotRef) {
     ...
     const match = lcValue.substring(0, level);
   ...
   toggleLaserSearch = () => {
     const refs = this.collectRefs(this.state.searchFor);
     const matchWords = this.collectMatchWords(this.state.searchFor);
   ...
   findInTree(tree, level, searchFor, getRefs, includeCounts) {
     const match = searchFor.substring(0, level).toLowerCase();
   ...
   getSearchOptions(searchFor, includeCounts) {
     ...
     const opts = this.findInTree(this.state.searchTree, 1, searchFor, null, includeCounts);
     if (includeCounts) {
       // Sort on counts
   ...
   toggleSearch = () => {
     if (!this.state.searchIsOpen) {
       ...
       // Cleanup
       this.setState({
         searchIsOpen: false, searchStatus: this.state.searchTerms, searchFor: '', searchResults: null,
       });
       this.props.callback([], [], this.state.laserSearch);
     }
   }
   ...
   //
   renderSearchOptions() {
     const spaceLoc = this.state.searchFor.lastIndexOf(' ');
     const incrSearchFor = spaceLoc >= 0 ? this.state.searchFor.substring(spaceLoc + 1) : this.state.searchFor;
     const options = this.getSearchOptions(incrSearchFor, true);
     ...
     return (
       <div className="search-results" onMouseLeave={() => this.setState({ searchFor: `${this.state.searchFor} ` })}>
         { options.map((option, index) => <div className="search-results-element" onClick={() => this.onClickOption(option, spaceLoc)} key={index}>{option}</div>)}
       </div>
     );
   }
   ...
   onClickOption(option, spaceLoc) {
     const optionWord = option.substring(0, option.indexOf(' '));
   ...
   render() {
     return (
       <div className="search">
         ...
         { this.state.searchIsOpen && (
         <button
           className={this.state.laserSearch ? 'search-laser-button-on' : 'search-laser-button-off'}
